...
<body>
	...
	<form action="/customer/signup" method="post">
		...
		비번 : <input type="password" name="pw"><br> 비밀번호 확인 : <input
	...
	<script>
		...
		btn_checkDupId.addEventListener('click', ()=>{
			// 아이디 중복체크 버튼 누르면 입력한 아이디 값을 확인
			// -> 서버로 ajax 요청 -> 중복 여부 확인
			...
			$.ajax({
				...
				//dataType: 'text',
				dataType: 'json',	//데이터 타입을 json으로 해두면 파싱할 필요가 없음. 이미 파싱이 된게 result에 들어가서..
				success: function(result){
					...
					//let jsObj = JSON.parse(result);	//파싱
					let jsObj = result;
