...
public class UserServiceImpl implements UserService {
	...
	public int saveAdminUser(User user) {
		...
		int result = userDAO.saveUser(user);
		log.info("savaAdminUser 관리자 계정 추가 : {}", user);
	...
	public int saveCustomerUser(User user) {
		...
		int result = userDAO.saveUser(user); // 암호화 된 비밀번호가 저장됨
	...
	public User checkUserLogin(User user) {
		 ...
		 * 
		 * // 암호화 처리가 되어 있으면? // loginUser.getPw().equals( user.getPw() ) //
	...
	public boolean isDuplicatedId(String id) {
		...
		if (user == null) { // 해당 아이디 객체가 없다 -> id값 사용 안한다 -> 중복 xxx
			return false;
		} else { // 아이디 객체가 잆다 -> id 값이 pk로 사용되고 있다 -> 중복 ooo
			return true;
		}
