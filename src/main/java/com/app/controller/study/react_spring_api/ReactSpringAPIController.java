...
public class ReactSpringAPIController {
	...
	@GetMapping("/api/getDrinkDiv")
	public List<DrinkItem> getDrinkDiv(HttpServletRequest request){
		...
		if(type != null) {
			if(type.equals("커피")) {
				drinkList.add(new DrinkItem("아메리카노", "커피"));
				drinkList.add(new DrinkItem("카페라떼", "커피"));
				drinkList.add(new DrinkItem("연유라떼", "커피"));
			}
			if(type.equals("차")) {
	...
	public String loginCheckJWT(HttpServletRequest request) {
		...
		// ---> 필요한 로직 수행 (id 기반으로 DB 조회) ㅇㅇㅇㅇ처리 -> return
		...
		//유효한 토큰이면 그 토큰 안에 저장된 사용자 id를 확인해볼 수 있음
