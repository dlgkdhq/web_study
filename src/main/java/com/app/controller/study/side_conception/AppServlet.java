...
public class AppServlet {
	...
	// 웹에서 요청이 들어왔을 때 최전방 인포데스크 직원
	protected void service(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		// 어떤 주소로 요청을 했는지 주소를 확인해야함
		...
		if (requestURI.equals("접속경로비교")) {
			...
			// viewName model: map<String, Object> 가 mav 안에 들어있음
		...
		if (requestURI.equals("접속경로비교")) {
			...
			// request에 담겨있는 parameter 값들을 전달하기 위해 Map<String, String> 형태로 변환
			// request -> map

			Enumeration<String> paramNames = request.getParameterNames(); // 파라미터 이름(key값)들만 모여있음
